...
class UserService extends Service {
    async addUser() {
        try {
            ...
            let oncepass = Cryptos.decode(userlist.password)
            let obj = {
    ...
    async delUser() {
        ...
        let secretOrPrivateKey = "zhangsuqi"
        let returnlist = null
    ...
    async loginUser() {
        ...
        try {
            ...
            if (global[userlist.date] == userlist.imgCode) {
                ...
                try {
                    returnlist.id
                    let pass = Md5s.decode(returnlist.password, returnlist.key)
    ...
    async updateUser() {
        ...
        if (returnlist) {
            ...
            if (returnlist.password == Cryptos.decode(obj.password)) {
                let updatelist = await this.app.mysql.update("users", {
                    id: returnlist.id,
                    loginName: returnlist.loginName,
                    status: returnlist.status,
